...
class CropWindow(QMainWindow):
    """this class creates a main window to observe the rowth of a simulated crop"""
    ...
    def __init__(self):
        ...
        #stacked layout
        self.stacked_layout = QStackedLayout() #holds various layout
    ...
    def create_select_crop_layout(self):
        #this ceates the initial layou of the window to select the crop type
    ...
    def create_view_crop_layout(self, crop_type):
        ...
        #set the properties of the crop view
        ...
        #add line edit widget to the status layout
    ...
    def instantiate_crop(self):
        crop_type = self.crop_radio_buttons.selected_button() #returns the button that was selected
        if crop_type == 1:
            self.simulated_crop = Wheat()
        elif crop_type == 2:
            self.simulated_crop = Potato()

        self.create_view_crop_layout(crop_type) #creates the view crop growth layout
        self.stacked_layout.addWidget(self.view_crop_widget)#add this to the stack
        self.stacked_layout.setCurrentIndex(1) #change the visible layout in the stack to index 1
...
def main():
    ...
    crop_window.show() #shows the instance
    crop_window.raise_() #put the instance at the fron of the screen
    crop_simulation.exec_() #monitors applicaiton for events
